	...
	class RolesModel extends Mysql
	{
		...
		*/
		...
		public function insertRol(string $rol, string $descripcion, int $status)
		{
			...
			$sql = "SELECT * FROM t_Rol WHERE nombre_rol = '{$this->strRol}'";
			$request = $this->select_all($sql);
			...
			if (empty($request))
			{
				...
				$query_insert = "INSERT INTO t_Rol(nombre_rol,descripcion,estatus) VALUES (?,?,?)";
		...
		public function updateRol (int $idrol, string $rol, string $descripcion, int $status)
		{
			...
			$sql = "SELECT * FROM t_Rol WHERE nombre_rol = '$this->strRol' AND id_rol != $this->intIdrol";
			$request = $this->select_all($sql);

			if (empty($request))
			{
				$sql = "UPDATE t_Rol SET nombre_rol = ?, descripcion = ?, estatus = ? WHERE id_rol = $this->intIdrol";
				$arrData = array($this->strRol, $this->strDescripcion,$this->intStatus);
