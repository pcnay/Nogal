			...
			if (request.readyState == 4 && request.status == 200) // Verifica que si llego la información.
			{
				...
				if (objData.status)
				{
					...
					tableRoles.api().ajax.reload() //function(){
						// Se colocan en comentarios ya que tiene problemas para recargar los botones.
...
function fntDelRol(id_rol)
{
	...
	*/
	...
	swal({
		title:"Eliminar Rol",
		text:"Realmente Desea Eiminar El Rol ?",
		type:"warning",
		...
		},function(isConfirm)
		{
			...
			if (isConfirm)
			{
				...
				request.onreadystatechange = function(){
					...
					{
						let objData = JSON.parse(request.responseText); // Lo convierte a un objeto.
						if (objData.status)
						{
